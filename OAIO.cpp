...
bool OAReader::readDesign(PowerOpt *po)
{
  ...
  try {
    ...
    while(oaInst * inst = instIter.getNext())
    {
      ...
      //oaCollection<oaInstTerm, oaInst> coll = inst->getInstTerms(); 
      
//      oaCollection<oaInstTerm, oaInst>::iterator it = coll.begin();
//      
//      for (; it!= coll.end() ; it++) { }
      ...
      while(oaInstTerm* instTerm = instTermIter.getNext())
      {
        ...
        //cout << "\tInst Term Name :" << termName << endl;  
      }
...
//					cout<<"  isBitInst ()  ?"<<inst->  isBitInst ()  <<endl;
    ...
    oaIter<oaTerm> termIter(design->getTopBlock()->getTerms(oacTermIterAll));
...
//					cout<<"number of terms: "<<net->getTerms().getCount()<<endl;
  ...
  cout << "Num Terminals : "  << po->getTerminalNum() << endl;
