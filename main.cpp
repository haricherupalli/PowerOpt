...
/*    if (!reader.readDesign(po)) {
    ...
    if (po->getExeOp() == 21) // NETLIST SIMULATION (X_based)
    {
      ...
      po->readConstantTerminals();// my_handler(0);
      po->readIgnoreNetsFile();// my_handler(0);
      po->createSetTrie();
      po->print_gates();
      po->print_nets();
      po->print_regs();
      ...
      po->simulate3();
      t4  = time(NULL);
      po->dump_all_toggled_gates_file();
      if (po->postprocess())
        po->simulation_post_processing(&T);
      ...
      system_state* dummy;
      long num_sys_states_created = dummy->get_sys_state_count();
      cout << " Num_sys_states_created = " << num_sys_states_created << endl;
